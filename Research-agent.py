...
# Define a subclass of PubmedQueryRun to include the is_single_input attribute
class PubmedQueryRunWithSingleInput(PubmedQueryRun):
    is_single_input = True  # Ensure compatibility with LangChain agent

# Use the new subclass
pubmed = PubmedQueryRunWithSingleInput(api_wrapper=pubmed_wrapper)
...
# Wrap Google Scholar query function as a LangChain Tool
...
google_scholar_tool.is_single_input = True  # Ensure compatibility with LangChain agent
...
if prompt := st.chat_input("Search me recent 5 years articles on role of oxytocin in prevention of PPH"):
    ...
    search_agent = initialize_agent(
        ...
        handle_parsing_errors=True  # Corrected typo
    )
